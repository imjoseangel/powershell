...
$a.WindowTitle = "Applies the Default Retention Policy to Mailboxes"

Clear-Host


#Setup LogFile file name
$LogFile = ".\$((Get-Date -uformat %Y-%m-%d_%H.%M.%S).ToString())_BeamMRMPolicy_LogFile.txt"
...
Get-Mailbox -Identity $user -ResultSize unlimited | Set-Mailbox -RetentionPolicy "Beam MRM Policy" >> $logfile
